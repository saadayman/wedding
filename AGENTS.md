...
Detected setup: Next.js App Router with TypeScript, Tailwind CSS v4, ESLint, and npm.
...
No `test`, `typecheck`, or `verify` script is configured yet. Automatic GitHub
checks are not configured; run `/ci` or `$ci` when you want one local Verify
command and matching GitHub Actions setup.
